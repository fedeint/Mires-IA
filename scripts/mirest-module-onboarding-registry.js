...
export const MIREST_MODULE_ONBOARDING = {
  ...
  proveedores: { id: "proveedores", label: "Proveedores", icon: "🚚", storageKey: "mirest_onb_done_proveedores", steps: [
    { paso: 1, titulo: "¿Quién te abastece?", descripcion: "Registra al menos un proveedor frecuente.", accion: "Crear ficha: nombre, RUC/tel, categoría.", completado_cuando: "≥1 fila en `proveedores` del tenant.", element: "main, #proveedoresRoot, body", verifyId: "provUno" },
    { paso: 2, titulo: "Datos de contacto y crédito", descripcion: "Completa días de crédito; suma al menos un tel o validación de contacto.", accion: "En la ficha, asegurar días de crédito (no nulo) y contacto operativo.", completado_cuando: "≥1 `proveedores` con `dias_credito` not null (0 cuenta).", element: "body", verifyId: "provCredito" },
    { paso: 3, titulo: "¡Proveedores listos!", descripcion: "Ahora carga o sincroniza insumos y movimientos en Almacén.", accion: "Ir a Almacén → Inventario / entradas.", completado_cuando: "Paso 2 completado (cierre de flujo).", element: "body", verifyId: "alwaysTrue" },
  ]},
  almacen: { id: "almacen", label: "Almacén", icon: "📦", storageKey: "mirest_onb_done_almacen", steps: [
    { paso: 1, titulo: "Registra tus insumos", descripcion: "Los insumos conectan costos, recetas y salidas a cocina.", accion: "Crea 3+ insumos mínimo con unidad, costo y mínimo.", completado_cuando: "≥3 `insumos` del tenant.", element: "main, body", verifyId: "ins3" },
    { paso: 2, titulo: "Define el stock mínimo", descripcion: "Cada fila con stock mínimo >0 habilita alertas.", accion: "Ajusta stock mínimo en el catálogo o edición de insumos.", completado_cuando: "Ningún `insumos` con stock_minimo 0 o null (del tenant), si hay al menos 1 fila.", element: "body", verifyId: "insStockMin" },
    { paso: 3, titulo: "Inventario inicial (movimientos)", descripcion: "Registra al menos un movimiento de entrada (compra, inventario o apertura) en almacen_movimientos.", accion: "Desde el flujo de entradas o ajuste que alimenta `almacen_movimientos` tipo `entrada*`.", completado_cuando: "≥1 fila `almacen_movimientos` tipo `entrada%` del tenant.", element: "body", verifyId: "almEntrada" },
    { paso: 4, titulo: "Verifica el stock", descripcion: "Revisa `stock_actual` o vista de saldo por insumo; debe haber al menos un saldo >0.", accion: "Ajusta entradas hasta ver stock positivo.", completado_cuando: "≥1 `insumos` con `stock_actual` >0.", element: "body", verifyId: "insStockPos" },
    { paso: 5, titulo: "¡Almacén listo!", descripcion: "Continúa con productos a la carta y luego recetas (mapeo plato–insumo).", accion: "Ir a Productos.", completado_cuando: "Cierre: paso 4 ok.", element: "body", verifyId: "alwaysTrue" },
  ]},
  ...
  facturacion: { id: "facturacion", label: "Facturación", icon: "🧾", storageKey: "mirest_onb_done_facturacion", steps: [
    { paso: 1, titulo: "Emisión y estados", descripcion: "Aquí conectas comprobantes con ventas. Revisa el panel y la cola de documentos del día.", accion: "Recorre la pantalla y confirma que el local tenga RUC/ datos de emisor en Configuración.", completado_cuando: "Navegación mínima completada; o confirma con «Siguiente» si aún no hay tráfico en vivo.", element: "main, body", verifyId: "alwaysTrue" },
    { paso: 2, titulo: "Cierre y conciliación", descripcion: "Luego alinea caja, reportes y retenciones según tu regulación.", accion: "Cierre del tour; continúa con Caja/Reportes.", completado_cuando: "Cierre: paso 1 ok.", element: "body", verifyId: "alwaysTrue" },
  ]},
